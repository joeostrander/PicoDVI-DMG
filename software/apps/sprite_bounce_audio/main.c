...
// 	{
// 		// const int32_t capture_value = sample_buffer[sample_count % 2048];
// 		capture_value = sample_buffer[sample_count % 2048];
// 		// const int32_t capture_value = sample_buffer[sample_count];
//     	// int32_t output_value = capture_value;
//     	output_value = capture_value;
// 		output_value = map(output_value, 0, 4095, INT16_MIN, INT16_MAX);
// 		// output_value = map(output_value, 0, 4095, -25000, 25000);
// 		// output_value -= 2048;
// 		// output_value *= 20;
// 		// output_value = output_value > 32767 ? 32767 : output_value;
// 		// output_value = output_value < -32768 ? -32768 : output_value;
//         sample.channels[0] = output_value;
//         sample.channels[1] = output_value;

//         *audio_ptr++ = sample;
//         sample_count++;
// 		// sample_count = sample_count > 255 ? 0 : sample_count;
// 		// sample_count = sample_count > 2048 ? 0 : sample_count;
//     }
...
// TODO: make sure buffer doesn't have some crap in it... 
// ------------------------------------------------------------------------------
 // initialize the analog microphone
	...
	// uint slice_num_led = pwm_gpio_to_slice_num(PWM_LED_PIN);
	// pwm_set_clkdiv(slice_num_led, 4.f);
